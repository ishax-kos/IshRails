...
public abstract class AbstractIshRailBlock extends Block implements IAbstractIshRailBlock  {
    ...
    public static boolean isRail(BlockState state) {
        return state.func_235714_a_(BlockTags.RAILS) && state.getBlock() instanceof AbstractIshRailBlock;
    }

    protected AbstractIshRailBlock(boolean p_i48444_1_, AbstractBlock.Properties p_i48444_2_) {
        super(p_i48444_2_);
    ...
    public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        IshRailShape IshRailShape = state.isIn(this) ? state.get(this.getShapeProperty()) : null;
        IshRailShape IshRailShape2 = state.getBlock() == this ? getRailDirection(state, worldIn, pos, null) : null;
        return IshRailShape != null && IshRailShape.isAscending() ? ASCENDING_AABB : FLAT_AABB;
    }
    ...
    public void neighborChanged(BlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos, boolean isMoving) {
        if (!worldIn.isRemote) {
            ...
            if (func_235328_a_(pos, worldIn, IshRailShape) && !worldIn.isAirBlock(pos)) {
                if (!isMoving) {
    ...
    private static boolean func_235328_a_(BlockPos p_235328_0_, World p_235328_1_, IshRailShape p_235328_2_) {
    ...
    protected BlockState getUpdatedState(World worldIn, BlockPos pos, BlockState state, boolean placing) {
        if (worldIn.isRemote) {
            ...
            IshRailShape IshRailShape = state.get(this.getShapeProperty());
            return (new IshRailState(worldIn, pos, state)).func_226941_a_(worldIn.isBlockPowered(pos), placing, IshRailShape).getNewState();
        }
    }
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        ...
        return blockstate.with(this.getShapeProperty(), flag ? IshRailShape.EAST_WEST : IshRailShape.NORTH_SOUTH);
    }
